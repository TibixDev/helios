...
use helios_protocol::{
    HeliosEscapeCtxCreate, HeliosEscapeCtxDestroy, HeliosEscapeHeader, GUID_DEVINTERFACE_HELIOS_DATA1,
    GUID_DEVINTERFACE_HELIOS_DATA2, GUID_DEVINTERFACE_HELIOS_DATA3, GUID_DEVINTERFACE_HELIOS_DATA4,
    HELIOS_ESCAPE_CTX_CREATE, HELIOS_ESCAPE_CTX_DESTROY, IOCTL_HELIOS_CTX_CREATE,
    IOCTL_HELIOS_CTX_DESTROY, VIRTIO_GPU_CAPSET_VENUS,
};
...
fn open_helios() -> Result<Handle, String> {
    ...
    unsafe {
        ...
        if h == INVALID_HANDLE_VALUE {
            return Err(format!("CreateFile failed (err {})", GetLastError()));
        }
        Ok(h)
    }
}
...
fn main() {
    ...
    say!("helios_probe: Phase 3 IOCTL smoke test");
    say!("[1/4] opening GUID_DEVINTERFACE_HELIOS ...");
    ...
    say!("[2/4] IOCTL_HELIOS_CTX_CREATE (capset = VENUS={VIRTIO_GPU_CAPSET_VENUS}) ...");
    ...
    say!("[3/4] IOCTL_HELIOS_CTX_DESTROY (ctx_id = {ctx_id}) ...");
