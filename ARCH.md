...
**NEXT = Phase 5 (the Mesa venus ICD, §5)** — the real validator of the blob path. Implement `MAP_BLOB` (§6, the MDL/user-VA path with the IRQL split) alongside it, and add a `blob_id` field to `HeliosEscapeAllocBlob` first. Deferred: the WDFINTERRUPT + async-fence DPC (resolve the `STATUS_DEVICE_POWER_FAILURE` legacy-INTx question then). Detail: the `phase4-blob-plan` and `mesa-venus-icd-port` memories.
